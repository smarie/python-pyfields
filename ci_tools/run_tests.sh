...
if [ "${TRAVIS_PYTHON_VERSION}" = "3.5" ]; then
   ...
   echo -e "\n\n****** Running tests ******\n\n"
   coverage run --source pyfields -m pytest --junitxml=reports/junit/junit.xml --html=reports/junit/report.html -v pyfields/  # and not pyfields/tests/: we want DocTest too
   # buggy
   ...
   # faster - skip coverage and html report but keep junit (because used in validity threshold)
    echo -e "\n\n****** Running tests******\n\n"
    python -m pytest --junitxml=reports/junit/junit.xml -v pyfields/  # and not pyfields/tests/: we want DocTest too
fi
